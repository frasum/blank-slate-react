...
Nachtrag Geofencing M1 + Mitarbeiter-Self-Service (umgesetzt 17.06.2026)

Geofencing M1: UI-Stempelungen sind server-seitig geofence-gegated (`src/lib/geo/`). `locations` erhielt `latitude`/`longitude`/`geofence_radius_m` (Default 100 m). `clockIn` verlangt (a) genau EINEN distinkten Standort in `staff_locations` (`pickSingleLocation` zählt distinkte `location_id`, nicht Zeilen) und (b) hinterlegte Koordinaten — sonst sprechende Ablehnung, kein Eintrag. Manager-Korrekturen sind geofence-frei. Bewusste Erweiterung über R4 hinaus (R4 erlaubte Geolocation, schrieb Geofencing nicht vor). VERTRAGSÄNDERUNG zu B2a: früher war `location_id NULL` beim Stempeln erlaubt; jetzt ist ein eindeutiger, geocodierter Standort Voraussetzung fürs UI-Stempeln.
