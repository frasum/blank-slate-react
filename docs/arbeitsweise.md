...
## 55. Schichttausch TA1 — Anfragen ohne automatischen Vollzug (04.07.2026)

Mitarbeiter können eigene, in der Zukunft liegende `roster_shifts` zum
Tausch anbieten; berechtigte Kollegen können sie übernehmen. TA1 legt
ausschließlich Anfragen an — **es gibt KEINEN automatischen Vollzug**.
Das Umschreiben von `roster_shifts` erfolgt erst durch die Manager-
Genehmigung in TA2.

**Zustandsmaschine:** `open` → `peer_accepted` → `approved` | `rejected`.
`cancelled` ist nur durch den **Anfragenden** und nur solange der Manager
noch nicht entschieden hat möglich. Der Kollege kann seine Annahme in v1
**nicht** zurückziehen (kleine Zustandsmaschine, Manager kann ablehnen).

**Berechtigten-Regel** (rein, `swap-rules.ts`): gleicher Standort UND
gleicher Arbeitsbereich (`staff_locations.department` == `roster_shifts.area`,
`gl` wird wie jeder andere Bereich behandelt), Kollege ist aktiv, kein
Tages-Konflikt (keine eigene Schicht am selben Datum am gleichen
Standort+Bereich). Optionaler Gegentausch wird nach denselben Regeln plus
Zukunfts- und Doppel-Anfrage-Prüfung geprüft.

**Datenmodell:** `shift_swap_requests` (id, organization_id, shift_id,
requester_staff_id, peer_staff_id?, peer_shift_id?, status, note, Zeiten).
Partieller Unique-Index `shift_swap_requests_active_shift` auf `shift_id`
WHERE `status IN ('open','peer_accepted')` — nur EINE aktive Anfrage pro
Schicht. §51-Anmerkung: der partielle Index ist hier zulässig, weil er
NICHT als `onConflict`-Ziel dient (Eindeutigkeit wird server-seitig
zusätzlich in `createSwapRequest` vor-geprüft; ein 23505 aus dem Index
wird in eine sprechende Fehlermeldung gemappt).

**Sicherheit:** RLS ist eingeschaltet, es gibt **keine Client-Policies**
(DENY-ALL). Alle Zugriffe laufen über `swap.functions.ts` mit
`supabaseAdmin` nach `loadStaffCaller`-Identität (`auth.uid`, nie aus
dem Client-Payload). `assertShiftDateUnlocked` verhindert Anfragen in
bereits gesperrte Perioden. `acceptSwapRequest` ändert **keine**
`roster_shifts` — der Vollzug bleibt TA2 vorbehalten.

**Status:** TA1 ✅ / TA2 offen (Manager-Genehmigung, roter Punkt im
Admin-Navi, Telegram-Notifications).
