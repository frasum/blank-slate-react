...
Stand: 07.07.2026 (BFIX3–7/SP2/P2 grün)
...
| P2 Finalize-E2E (Kassen-Finalize Playwright-Rundgang, Seed-Cluster)                                                                                                                                   | ✅ code-seitig; 3/3-Lauf via CI bzw. nach lokalem Docker-Fix                                                                                   |
...
## 72. Migrations-Replayfähigkeit, E2E-Härtung & Schema-Parität (BFIX3–7, P2b–f, SP2, 07.07.2026)
...
**Offen (nächste Session, dokumentierter Einstieg).** Der lokale 3/3-E2E-Beweis steht aus — auf Franks Mac existiert ein Docker-Doppelwelt-Phänomen: CLI-Resets liefen zweimal vollständig durch, die am Port 54321 hängende DB blieb dennoch auf einem 56-Tabellen-Teilstand; `docker context default` vs `desktop-linux` unterscheiden sich. Zusatz-Anomalie beim direkten `psql`-Replay: `has_role` in `20260616145016` nicht gefunden, obwohl `20260614163243` sie erzeugt. Diagnose-Startpunkte: `docker --context default ps -a` gegen `desktop-linux` vergleichen, Docker Desktop als einzige Engine durchsetzen; danach reicht die dokumentierte Sequenz (reset → SIGUSR1 → Test). **Alternativer Beweisweg ohne Mac-Docker:** der CI-`e2e`-Job — gleiche Kette, eine Welt; nach den heutigen Commits auf GitHub Actions grün prüfen (dann ist P2 bewiesen und das Promotions-Kriterium tickt).
