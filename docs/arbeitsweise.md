...
### Kachel-Sichtbarkeit deckungsgleich (via gemeinsame Helper)

Die „Abrechnung"-Kachel im `/zeit`-Hub (`canSeeAbrechnungTile`) ist sichtbar **genau dann**, wenn die Eröffnung tatsächlich erlaubt wäre — nicht großzügiger. Dazu teilen sich die Kachel und die harte Grenze eine **Server-Helper** `resolveMySessionLocation` (`src/lib/cash/session-location.server.ts`), die die DB-Inputs sammelt (`role`, `staff_locations`, für staff `roster_shifts area='service'` am Geschäftstag — alle auf den Caller gescoped) und die reine `resolveSessionLocation` aufruft. Beide Aufrufer nutzen nur noch diese Helper: `ensureMyOpenSession` mappt das Ergebnis auf Eröffnung/Fehlermeldung, `canSeeAbrechnungTile` gibt `visible = resolved.ok` zurück (defensiv fail-closed bei Fehlern). Damit können Kachel-Sichtbarkeit und Eröffnungs-Regel **strukturell nicht mehr auseinanderlaufen**. Die harte Grenze bleibt `ensureMyOpenSession`; die Kachel ist nur UI-Kosmetik.
