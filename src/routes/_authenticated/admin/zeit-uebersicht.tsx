...
function ZeitUebersichtPage() {
  ...
  // Flache Liste aller sichtbaren Zeilen (für Tab/Pfeil-Navigation).
  ...
  // Sucht die nächste editierbare (row, day, field) Kombination.
  // dir: "next" | "prev". mode: "row" (nur Zeile ändern), "field" (Feld/Tag).
  const findNext = (
    rowIdx: number,
    dayIdx: number,
    field: "from" | "to",
    ...
    for (let guard = 0; guard < 200; guard++) {
      if (mode === "row") {
        r += step;
        if (r < 0 || r >= flatRows.length) return null;
      } else {
        // field-mode: from → to → from(nextDay) → to → …
        if (dir === "next") {
          if (f === "from") f = "to";
          else {
            f = "from";
      ...
      if (isCellEditable(flatRows[r], d)) {
        return { rowIdx: r, dayIdx: d, field: f };
      }
    }
    return null;
  };

  const startEditFromTarget = (target: {
    rowIdx: number;
    dayIdx: number;
    field: "from" | "to";
  }) => {
    const row = flatRows[target.rowIdx];
    const iso = dayMeta[target.dayIdx].iso;
    startEdit(row.staffId, iso, target.field);
  };
