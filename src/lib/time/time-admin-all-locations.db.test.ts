...
describe.skipIf(!dbTestsEnabled)("getTimeOverview — Alle-Standorte-Sicht & Lücken (WZ1)", () => {
  let org: SeededOrg;
  ...
  function sumHours(rows: ReadonlyArray<{ started_at: string; ended_at: string | null }>): number {
  ...
  it("(b) Einträge mit location_id IS NULL sind in der Alle-Standorte-Sicht enthalten", async () => {
    const rows = await selectEntriesOrgWide();
    const nullLoc = rows.filter((r) => r.staff_id === staff.staffId && r.location_id === null);
  ...
  it("(c) Standort-Sicht liefert gaps.unlocatedShifts=1 und gaps.openShifts=1", async () => {
    ...
    expect(closedAtLoc1.some((r) => r.business_date === "2027-08-08")).toBe(false);
