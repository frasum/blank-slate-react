...
describe.skipIf(!dbTestsEnabled)("cash read endpoints (DB) — B3c-1a", () => {
  ...
  beforeAll(async () => {
    ...
    const { data: ch1 } = await org.service
      ...
      .insert({ organization_id: org.orgId, label: "Restaurant", sort_order: 1 })
      .select("id")
      .single();
    await org.service
      .from("revenue_channels")
      .insert({ organization_id: org.orgId, label: "Lieferung", sort_order: 2 });
    await otherOrg.service
      .from("revenue_channels")
      .insert({ organization_id: otherOrg.orgId, label: "FREMD", sort_order: 1 });
    channelId = ch1!.id;

    const { data: t1 } = await org.service
      .from("payment_terminals")
      .insert({ organization_id: org.orgId, label: "Terminal A", sort_order: 1 })
      .select("id")
      .single();
    await otherOrg.service
      .from("payment_terminals")
      .insert({ organization_id: otherOrg.orgId, label: "FREMD-T", sort_order: 1 });
    terminalId = t1!.id;
