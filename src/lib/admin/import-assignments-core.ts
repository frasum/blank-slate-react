...
export async function runImportAssignmentsCore(input: ImportCoreInput): Promise<ImportCoreResult> {
  ...
  const mapRows = expectOk<{ alt_id: string; staff_id: string | null; confirmed_at: string | null }[]>(
    ...
    "runImportAssignmentsCore.identityMap",
  );
  ...
  const locRows = expectOk<{ id: string }[]>(
    await admin.from("locations").select("id").eq("organization_id", organizationId),
    "runImportAssignmentsCore.locations",
  );
  ...
  if (staffIds.length > 0) {
    const slRows = expectOk<{ staff_id: string; location_id: string; department: string }[]>(
      ...
      "runImportAssignmentsCore.staffLocations",
    );
    ...
    const ssRows = expectOk<{ staff_id: string; skill_id: string }[]>(
      await admin
        .from("staff_skills")
        .select("staff_id, skill_id")
        .eq("organization_id", organizationId)
        .in("staff_id", staffIds),
      "runImportAssignmentsCore.staffSkills",
    );
  ...
  if (adds.length > 0) {
    expectVoid(
      ...
      "runImportAssignmentsCore.staff_locations.insert",
    );
  }
  for (const o of plan.locationOps) {
    ...
    expectVoid(
      ...
      "runImportAssignmentsCore.staff_locations.delete",
    );
  }
  ...
  if (skillAdds.length > 0) {
    expectVoid(
      ...
      "runImportAssignmentsCore.staff_skills.insert",
    );
  }
  for (const o of plan.skillOps) {
    ...
    expectVoid(
      ...
      "runImportAssignmentsCore.staff_skills.delete",
    );
  }
