...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM cart_items WHERE cart_id = v_cart.id AND supplier_id IS NULL
  ) THEN
    RAISE EXCEPTION 'Cart-Item ohne Lieferant.' USING ERRCODE = 'P0005';
  END IF;

  FOR v_supplier IN
    SELECT DISTINCT supplier_id FROM cart_items WHERE cart_id = v_cart.id
  LOOP
    INSERT INTO orders (
...
REVOKE ALL ON FUNCTION public.create_order_from_cart(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_order_from_cart(uuid, uuid, text) TO service_role;